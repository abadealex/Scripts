...
# Global OCR progress state (in-memory; use Redis or DB in production)
ocr_state = {
    "progress": 0
}
...
@file_bp.route('/start-ocr', methods=['POST'])
def start_ocr():
    """
    Starts OCR processing (simulated with a background thread).
...
@file_bp.route('/ocr-progress', methods=['GET'])
def ocr_progress():
    """
